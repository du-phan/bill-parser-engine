"""
Run TargetArticleIdentifier and OriginalTextRetriever on chunk(s).

This focuses on Steps 2–3 for rapid iteration and validation of identification
and retrieval behavior with real calls.

Usage examples:
  - Single chunk:
      poetry run python scripts/run_identifier_and_retriever.py \
        --chunk scripts/output/duplomb_chunks_all/071___TITRE_IV_Article_8_III_11_b_.txt

  - All chunks in a directory (with JSONL log):
      poetry run python scripts/run_identifier_and_retriever.py \
        --dir scripts/output/duplomb_chunks_all \
        --log-file scripts/output/validation_logs/identify_retrieve.jsonl

Notes:
  - Requires .env/.env.local with MISTRAL_API_KEY (for identifier and any LLM fallback).
  - Uses local stores only for French/EU texts. No external API needed.
  - Expects a .json sidecar next to each .txt chunk.
"""
...
def _load_chunk(chunk_txt_path: Path) -> BillChunk:
    ...
    # Populate inherited_target_article using the splitter's inference, so identifier can apply deterministic guard
    ...
    # Extract code context from major subdivision or article intro
...
def process_single_chunk(identifier: TargetArticleIdentifier, retriever: OriginalTextRetriever, chunk_path: Path, log_path: Optional[Path]) -> Tuple[str, Optional[TargetArticle]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TargetArticleIdentifier and OriginalTextRetriever on chunk(s)")
    ...
    if args.chunk:
        ...
        _, ta = process_single_chunk(identifier, retriever, chunk_path, log_path)
        processed += 1
        if ta is None:
            pass
        elif ta.operation_type == TargetOperationType.OTHER:
            skipped += 1
        else:
            succeeded += 1
    else:
        ...
        for txt_path in _iter_chunk_txt_files(directory):
            _, ta = process_single_chunk(identifier, retriever, txt_path, log_path)
            processed += 1
            if ta is None:
                continue
            if ta.operation_type == TargetOperationType.OTHER:
